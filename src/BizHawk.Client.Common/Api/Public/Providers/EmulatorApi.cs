...
{
	public class EmulatorApi : ApiProvider
	{
		...
		[OptionalService]
		private IGameInfo GameInfo { get; set; }
		#endregion
		...
		public override IEnumerable<ApiCommand> Commands => new List<ApiCommand>()
		{
			...
			new ApiCommand("GetROMName", args=>GameInfo?.Name ?? throw new ApiError("No ROM is loaded"), new List<ApiParameter>(), "Returns the name of the currently loaded ROM, if a ROM is loaded"),
			new ApiCommand("GetROMHash", args=>GameInfo?.Hash ?? throw new ApiError("No ROM is loaded"), new List<ApiParameter>(), "Returns the hash of the currently loaded ROM, if a ROM is loaded")
		};

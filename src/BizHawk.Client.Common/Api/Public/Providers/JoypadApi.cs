...
{
	public class JoypadApi : ApiProvider
	{
		[RequiredService]
		...
		public override IEnumerable<ApiCommand> Commands => new List<ApiCommand>() {
			...
			new ApiCommand("QueueInput",
				args=>AddInputToQueue(new Input(ParseButtonString(args?.FirstOrDefault()), int.Parse(args?.ElementAtOrDefault(1) ?? "0"),int.Parse(args?.ElementAtOrDefault(2) ?? "0"), (args?.Count() ?? 0) > 3 ? string.Join("/", args?.Skip(3)) : null)),
		...
		private readonly Queue<Input> InputQueue = new Queue<Input>();
		...
		private void AddInputToQueue(Input input)
		{
			lock(InputQueue)
			{
		...
		{
			get => InputManager.AutofireStickyXorAdapter.Source.Definition.BoolButtons.Concat(new string[] { "Hold" });
			set
			{
				foreach (var button in value.Where(b => b != "Hold"))
				{
					InputManager.ButtonOverrideAdapter.SetButton(button, true);
				}
				InputManager.ActiveController.Overrides(InputManager.ButtonOverrideAdapter);
			}
		}

		public IEnumerable<string> PressedButtons => Buttons.Where(b => InputManager.AutofireStickyXorAdapter.IsPressed(b));
